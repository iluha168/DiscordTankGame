...
export const cmd: CMD = {
	AppCmdOption: {
        ...
        options: [{
            name: "right",
            description: "How many squares to go to the right? Negative numbers allowed!",
	...
	[ApplicationCommandTypes.ChatInput]: async (intr)=>{
        const errMsg = await intr.db.moveTank(intr.interaction.user.id, intr.options.right as number, intr.options.down as number)
        await intr.reply(
            errMsg ?
            msgFail(errMsg):
