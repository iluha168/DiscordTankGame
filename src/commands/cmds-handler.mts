...
async function executeCmdInteraction(cmd: CMD, intr: InteractionWrapper){
    if(cmd.adminOnly){
        if(!await intr.isRequestedByAdmin()) return intr.reply({
            embeds: [{
                description: 'You must be admin of the game in order to run this command.',
                color: 0xFF0000
            }]
        })
    }
